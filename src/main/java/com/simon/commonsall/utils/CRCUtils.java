package com.simon.commonsall.utils;

/** 
 * <pre>
 * CRCUtils
 * </pre>
 * @author  ZhangShanshan
 * @date    2017年2月27日
 * @version 1.0 
 */
public class CRCUtils {
	static final int gu8CrcTable[] = { , };

	/**
	 * 求CRC
	 * @author ZhangShanshan
	 * @date   2017年2月27日
	 * @param pData
	 * @param u32Length
	 * @return
	 */
	public static byte CRC8(byte[] pData, int u32Length) {
		return CRC8(pData,0,u32Length);
	}
	
	/**
	 * 
	 * @author ShiXiaoyong
	 * @date   2017年3月3日
	 * @param pData
	 * @param offset
	 * @param u32Length
	 * @return
	 */
	public static byte CRC8(byte[] pData,int offset, int u32Length) {
		byte u8CrcResult = 0;
		
		for (int i = offset; i < offset+u32Length; i++) {
			int index = (u8CrcResult & 0xff) ^ (pData[i]&0xff);
			u8CrcResult = (byte) gu8CrcTable[index];
		}
		
		return u8CrcResult;
	}
}
